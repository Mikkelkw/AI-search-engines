...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + directionx), int(y + directiony)
    ...
    def getCostOfActions(self, actions):
        ...
        for action in actions:
            directionx, directiony = Actions.directionToVector(action)
            x, y = int(x + directionx), int(y + directiony)
            if self.walls[x][y]: return 999999
        return len(actions)
...
class FoodSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextF = state[1].copy()
                nextF[nextx][nexty] = False
                successors.append( ( ((nextx, nexty), nextF), direction, 1) )
        return successors
...
def foodHeuristic(state, problem):
    ...
    for n in range(0, len(foodList)):
        currFood = foodList[n]
        currFoodDist = util.manhattanDistance(position, currFood)

        if currFoodDist > dist:
            farP = currFood
            dist = currFoodDist
    if (position, farP) in problem.heuristicInfo:
        return problem.heuristicInfo[(position, farP)]
    else:
        heuristic = dist
        problem.heuristicInfo[(position, farP)] = heuristic
        return heuristic
